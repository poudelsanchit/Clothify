...
const EachCard = ({ items }) => {
  ...
  return (
    <div className=" flex h-32 w-full border-2 rounded-md  gap-5 ">
      ...
      <div
        className="flex h-full justify-center items-center cursor-pointer text-3xl"
        onClick={() => dispatch(removeItemFromCart(items.productId))}
      >
        <CiTrash />
      </div>
      <div className="hidden lg:flex justify-center items-center h-full ml-auto font-Poppins font-medium  text-lg w-20  text-black pr-4 rounded-r-md ">
        {" "}
        ${totalPrice}
      </div>
    </div>
