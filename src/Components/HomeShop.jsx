...
const HomeShop = () => {
  ...
  const fetchData=async()=>{
    const ProductsData = await axios.get('https://done-hejj.onrender.com/products')
    setProducts(ProductsData.data)

  }
  const [products2,setProducts2]= useState([])
  const fetchProducts2=async()=>{
    const ProductsData = await axios.get('https://done-hejj.onrender.com/products2')
    setProducts2(ProductsData.data)

  }
  const [products3,setProducts3]= useState([])
  const fetchProducts3=async()=>{
    const ProductsData = await axios.get('https://done-hejj.onrender.com/products3')
